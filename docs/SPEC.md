...
| **Cilium Hubble adapter** (Level 2) | High | When Cilium is detected on the cluster, the KubeBolt agent streams flows from Hubble Relay (gRPC) via an in-cluster DaemonSet and forwards normalized samples over its existing gRPC StreamMetrics pipe. A K8s Lease (`kubebolt-flow-collector` in the agent namespace) ensures only one agent pod consumes the stream at a time. Critical for SaaS: customers never expose Hubble externally. |
...
What landed across the Hubble-adapter cut (commits `feat(flows)` /
`feat(map)`) versus the full Phase 2.1 vision. Most follow-ups from
the original MVP have shipped; the remaining ones are enrichment
items, not correctness gaps.

| Area | State | Remaining work |
|------|-------|----------------|
| **Metric shape** | ✅ Done. `pod_flow_events_total` + L7 enrichment (`pod_flow_http_requests_total`, `pod_flow_http_latency_seconds_sum/count`), DNS resolutions (`pod_dns_resolutions_total`), and external flows (`pod_flow_external_events_total`). Hubble is event-oriented and doesn't report bytes per flow natively. | Optional: scrape Cilium's own Prometheus metrics (`cilium_forward_bytes_total`) to enrich edges with true byte rates. Not required — event rate + L7 RPS already cover the user-visible use case. |
| **Event dedup** | ✅ Done. Aggregator filters `IsReply=true` and keeps only `TrafficDirection=EGRESS` so a single HTTP round trip contributes one counter bump instead of four (request + reply × ingress + egress perspectives). L7 RESPONSE events get src/dst flipped back to caller-perspective before emission. | None. |
| **Source detection** | ✅ Done. Defaults to `hubble-relay.kube-system.svc.cluster.local:80`; `KUBEBOLT_HUBBLE_RELAY_ADDR` overrides. New `KUBEBOLT_HUBBLE_ENABLED` kill-switch (default on) lets operators disable flow collection without redeploying. Silent no-op when the relay Service doesn't resolve. | Optional: boot-time probe to emit a status sample distinguishing "no Cilium" from "collector crashed". Low value now that `hubble_collector_up` gauge covers the runtime case. |
| **L7 enrichment** | ✅ Done. Emits `pod_flow_http_requests_total{method,status}` and `pod_flow_http_latency_seconds_*` from L7 events. Cluster map edges color by HTTP status class (2xx green, 3xx violet, 4xx amber, 5xx red) with average latency in the tooltip. | None. |
| **Intent flows** | ✅ Done. Two complementary mechanisms: (a) cluster map Traffic layout routes edges caller → Service → callee by feeding dagre both `selects` topology edges and observed flows; (b) DNS resolutions observed via Hubble label external endpoints by FQDN instead of raw IP, surfaced as synthetic `ExternalEndpoint` nodes. | None for the MVP; HTTP Host-header intent is an optional future enrichment. |
| **Edge category filters** | ✅ Done. Six per-group toggles (Ownership / Service / Config / Storage / Autoscale / Traffic) with localStorage persistence. | None. |
| **Multi-cluster Hubble** | ✅ Done. `cluster_id` auto-discovered from the `kube-system` namespace UID at agent boot — guaranteed unique and immutable per cluster. `KUBEBOLT_AGENT_CLUSTER_ID` override for legacy migrations; `KUBEBOLT_AGENT_CLUSTER_NAME` for the human-readable label. Backend scopes all PromQL queries by the UID of the active kubeconfig context. | None. |
| **Security (TLS / mTLS)** | ✅ Done. Agent supports plain insecure, TLS (via `KUBEBOLT_HUBBLE_RELAY_CA_FILE`), and mTLS (add `KUBEBOLT_HUBBLE_RELAY_CERT_FILE` + `KEY_FILE`). `KUBEBOLT_HUBBLE_RELAY_SERVER_NAME` overrides SNI when needed. Covered by unit + integration tests with in-memory ECDSA certs. | None. |
| **Singleton coordination** | ✅ Done. Leader election via `coordination.k8s.io/Lease` (name `kubebolt-flow-collector` in the agent's own namespace). `kubebolt_flow_collector_leader{pod=...}` gauge published so the UI / ops can see who owns the stream. `hubble_collector_up` gauge tracks stream state independently of leadership. | Known artifact: dead leader's last value=1 can linger up to 5 min (VM lookback). Lease object remains source of truth; documented in `leader.go`. |
| **Operator controls** | ✅ Done. All agent env vars enumerated in `packages/agent/.env.example`; root `.env.example` points to it. Flag matrix: enable/disable (`KUBEBOLT_HUBBLE_ENABLED`), relay address, TLS/mTLS material, cluster identity overrides. | None. |
