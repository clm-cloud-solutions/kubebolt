...
| **Source detection** | Agent-side: defaults to `hubble-relay.kube-system.svc.cluster.local:80` (standard Cilium install) from inside the DaemonSet. `KUBEBOLT_HUBBLE_RELAY_ADDR` override for non-default namespaces / ports. Silent no-op when the relay Service doesn't resolve or Cilium isn't installed. | Probe the Hubble Relay Service on agent boot and emit a status sample when absent so the UI can tell "no Cilium" from "collector crashed". Support discovery of non-standard relay Service names via label selector. |
...
| **Multi-cluster Hubble** | Naturally multi-cluster: each cluster's agent DaemonSet streams flows tagged with its own `cluster_id` label (defaults to `"local"` in single-cluster dev; backend sets it per context in multi-cluster installs). Flows never cross cluster boundaries at the network layer. | Propagate the real `cluster_id` label from the backend kubeconfig context to the agent at registration time so labels match in multi-cluster SaaS deployments. |
| **Security (TLS / mTLS)** | Insecure gRPC from agent to Hubble Relay over the pod network; acceptable because the hop stays inside the cluster. | mTLS support for Cilium installs that require it on Hubble Relay. Mount client cert/CA into the agent Pod from a configurable Secret. |
| **Singleton coordination** | Leader election via `coordination.k8s.io/Lease` (name `kubebolt-flow-collector` in the agent's own namespace). 15s lease duration, 10s renew, 2s retry. Non-leader agents log the current leader and stand by. On leader eviction, another pod acquires within ~15s with at most a short gap in flow samples. | Publish the leader identity as a gauge (`kubebolt_flow_collector_leader{pod=...} 1`) so the UI can show which agent pod owns the stream, useful for debugging SaaS incidents. |
