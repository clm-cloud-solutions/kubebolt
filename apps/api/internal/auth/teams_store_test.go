...
func TestTeamStore_CreateGetList(t *testing.T) {
	...
	org := "org-1"
	a, err := ts.CreateTeam(context.Background(), org, "Platform")
	if err != nil {
	...
	if _, err := ts.CreateTeam(context.Background(), org, "Data"); err != nil {
		t.Fatalf("CreateTeam Data: %v", err)
	}
	got, err := ts.GetTeam(context.Background(), a.ID)
	if err != nil || got.Name != "Platform" {
		t.Fatalf("GetTeam = %#v, %v", got, err)
	}
	teams, err := ts.ListTeams(context.Background(), org)
	if err != nil || len(teams) != 2 {
		t.Fatalf("ListTeams = %d teams, %v", len(teams), err)
	}
	// Other orgs don't leak in.
	if other, _ := ts.ListTeams(context.Background(), "org-2"); len(other) != 0 {
...
func TestTeamStore_NameUniquePerOrg(t *testing.T) {
	ts := newTestTeamStore(t)
	if _, err := ts.CreateTeam(context.Background(), "org-1", "Platform"); err != nil {
		t.Fatalf("first: %v", err)
	}
	if _, err := ts.CreateTeam(context.Background(), "org-1", "platform"); !errors.Is(err, ErrTeamExists) {
		t.Fatalf("dup name (case-insensitive) should be ErrTeamExists, got %v", err)
	}
	// Same name in a DIFFERENT org is fine.
	if _, err := ts.CreateTeam(context.Background(), "org-2", "Platform"); err != nil {
...
func TestTeamStore_EnsureDefaultIdempotent(t *testing.T) {
	ts := newTestTeamStore(t)
	a, err := ts.EnsureDefaultTeam(context.Background(), "org-1")
	if err != nil || a.Name != DefaultTeamName {
		t.Fatalf("EnsureDefaultTeam = %#v, %v", a, err)
	}
	b, err := ts.EnsureDefaultTeam(context.Background(), "org-1")
	if err != nil {
	...
	if teams, _ := ts.ListTeams(context.Background(), "org-1"); len(teams) != 1 {
...
func TestTeamStore_Membership(t *testing.T) {
	ts := newTestTeamStore(t)
	team, _ := ts.CreateTeam(context.Background(), "org-1", "Platform")

	if _, err := ts.AddMember(context.Background(), team.ID, "user-A", RoleEditor); err != nil {
		t.Fatalf("AddMember: %v", err)
	}
	if _, err := ts.AddMember(context.Background(), team.ID, "user-B", ""); err != nil { // inherit
		t.Fatalf("AddMember inherit: %v", err)
	}
	// AddMember to a missing team fails.
	if _, err := ts.AddMember(context.Background(), "nope", "user-A", RoleViewer); !errors.Is(err, ErrTeamNotFound) {
		t.Fatalf("AddMember to missing team = %v, want ErrTeamNotFound", err)
	}

	m, ok, err := ts.GetMembership(context.Background(), team.ID, "user-A")
	if err != nil || !ok || m.TeamRole != RoleEditor {
		t.Fatalf("GetMembership = %#v, ok=%v, %v", m, ok, err)
	}
	members, _ := ts.ListMembers(context.Background(), team.ID)
	if len(members) != 2 {
		t.Fatalf("ListMembers = %d, want 2", len(members))
	}

	// Update the team_role (idempotent upsert).
	if _, err := ts.AddMember(context.Background(), team.ID, "user-A", RoleAdmin); err != nil {
		t.Fatalf("update member: %v", err)
	}
	if m, _, _ := ts.GetMembership(context.Background(), team.ID, "user-A"); m.TeamRole != RoleAdmin {
		t.Fatalf("team role not updated: %#v", m)
	}

	// ListUserTeams across teams.
	team2, _ := ts.CreateTeam(context.Background(), "org-1", "Data")
	ts.AddMember(context.Background(), team2.ID, "user-A", RoleViewer)
	ut, _ := ts.ListUserTeams(context.Background(), "user-A")
	if len(ut) != 2 {
		t.Fatalf("ListUserTeams(user-A) = %d, want 2", len(ut))
	}

	// Remove.
	if err := ts.RemoveMember(context.Background(), team.ID, "user-B"); err != nil {
		t.Fatalf("RemoveMember: %v", err)
	}
	if _, ok, _ := ts.GetMembership(context.Background(), team.ID, "user-B"); ok {
...
func TestTeamStore_DeleteDropsMemberships(t *testing.T) {
	ts := newTestTeamStore(t)
	team, _ := ts.CreateTeam(context.Background(), "org-1", "Platform")
	ts.AddMember(context.Background(), team.ID, "user-A", RoleEditor)
	if err := ts.DeleteTeam(context.Background(), team.ID); err != nil {
		t.Fatalf("DeleteTeam: %v", err)
	}
	if _, err := ts.GetTeam(context.Background(), team.ID); !errors.Is(err, ErrTeamNotFound) {
		t.Fatalf("team should be gone, got %v", err)
	}
	if members, _ := ts.ListMembers(context.Background(), team.ID); len(members) != 0 {
